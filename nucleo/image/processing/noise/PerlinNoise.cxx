/*
 *
 * nucleo/image/processing/noise/PerlinNoise.cxx --
 *
...
namespace nucleo {
...
#if DEBUG_LEVEL>0
  static double noise_min=0.0, noise_max=0.0 ;
#endif
  ...
  double ImprovedPerlinNoise::noise(double x, double y, double z) {
    ...
    if (result>noise_max) {
	 noise_max = result ;
	 std::cerr << "noise in [" << noise_min << ", " << noise_max << "]" << std::endl ;
    }
    if (result<noise_min) {
	 noise_min = result ;
	 std::cerr << "noise in [" << noise_min << ", " << noise_max << "]" << std::endl ;
  ...
  bool ImprovedPerlinNoise::initialized = false ;
  int ImprovedPerlinNoise::p[512] ;

  ImprovedPerlinNoise::ImprovedPerlinNoise(bool randomize) {
    if (!initialized) {
    ...
    if (randomize) {
	 biasx = (double)(random()%1000)/153.0 ;
	 biasy = (double)(random()%1000)/153.0 ;
	 biasz = (double)(random()%1000)/153.0 ;
    } else {
